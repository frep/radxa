...
tempDir=$kernelDir/build.radxa.com/rabian/nightly/rock/$1/rockdev/

targetDir=$kernelDir/rabian/$1/

radxaNightlyDir=http://build.radxa.com/rabian/nightly
radxaRockDir=$radxaNightlyDir/rock
...
function getFiles()
{
	wget -r --no-parent --reject "index.html*" $radxaRockDir/$1/rockdev/package-file
	wget -r --no-parent --reject "index.html*" $radxaRockDir/$1/rockdev/parameter
	wget -r --no-parent --reject "index.html*" $radxaRockDir/$1/rockdev/boot/boot-linux.img
	wget -r --no-parent --reject "index.html*" $radxaRockDir/$1/rockdev/modules/lib
}
